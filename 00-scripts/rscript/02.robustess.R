...
# Comparaison All
x <- as.factor(rep(c(1:4),each=nlineful2))
target.stat=c(8:13,20:41)
...
res=model_selection_abc_nnet(target=obs, x=x, sumstat=z, tol=500/(nrow(M.full)), noweight=F, rejmethod=F, nb.nnet=50, size.nnet=15, output=paste("02.results/robustess_",target, "_",lowbnd,sep=""))
