...
write.table(z,"abc.am",col.names=T,row.names=F,quote=F)
write.table(z[4,],"abc.mean.am",col.names=F,row.names=F,quote=F)

#plot graphe
pdf(file="pior.posterior.am.pdf",width=12,height=8)
par(mfrow=c(2,4))
...
hist(M_SC[,7],
	...
	freq=F,
	ylim=c(0,0.5),
	main="time of migration stop( 4N generations)",
	xlab= expression(tau),
	ylab="probab density")
...
abc.posterior <- abc(target  = obs2, param = M_SC[,param.im], sumstat = M_SC[,target.stat], tol = 0.001, transf=c("logit", "logit", "logit", "logit","logit", "logit" ), 
	logit.bounds = rbind(range(M_SC[, 2]), range(M_SC[, 3]), range(M_SC[, 4]), range(M_SC[, 5]),range(M_SC[, 6]),range(M_SC[, 7])),  hcorr=T, method  = "neuralnet", numnet=50, sizenet=15)

write.table(abc.posterior$weights,"weight.am_v2",quote=F,row.names=F,col.names=F)
write.table(abc.posterior$adj.values,"adjval.am_v2",quote=F,row.names=F)

z<-summary(abc.posterior)

write.table(z,"abc.am_v2",col.names=T,row.names=F,quote=F)
write.table(z[4,],"abc.mean.am_v2",col.names=F,row.names=F,quote=F)

pdf(file="pior.posterior.am_v2.pdf",width=12,height=8)
par(mfrow=c(2,3))
