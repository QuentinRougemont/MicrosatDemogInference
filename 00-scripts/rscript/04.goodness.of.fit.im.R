...
#estimate posterior
abc.posterior <- abc(target  = obs2, param = M_IM[,param.im], sumstat = M_IM[,target.stat], tol = 500/2e6, transf=c(rep("logit", 6) ), 
...
write.table(newsamp1,"ppc.im", quote=F,row.names=F,col.names=T)
...
nsim  <- nsim
add_simulations <- F
reftable_file   <- "reference_table_ppc.im.txt"
batch <- 1
...
ppc_im=read.table('reference_table_ppc.im.txt', skip=1)
...
for (i in 1:ncol(ppc_im)){
	...
	write.table(t(pval),file="ppc_IM_Goodness.txt", append=T,quote=F,row.names=mod_names[i], col.names=F)
	}
dev.off()
