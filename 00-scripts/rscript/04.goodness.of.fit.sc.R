...
#estimate posterior
abc.posterior <- abc(target  = obs2, param = M_SC[,param.sc], sumstat = M_SC[,target.stat], tol = 500/2e6, transf=c(rep("logit", 7) ), 
...
write.table(newsamp1,"ppc.sc", quote=F,row.names=F,col.names=T)
...
nsim  <- nsim
add_simulations <- F
reftable_file   <- "reference_table_ppc.sc.txt"
batch <- 1
...
ppc = read.table('reference_table_ppc.sc.txt', skip=1)
...
par(mfrow=c(6,7))
for (i in 1:ncol(ppc)){
	hist(ppc[,i], xlab=mod_names[i], main=mod_names[i])
	abline(v=obs[,i], col='red')
	pval=mean(abs(ppc[,i]) >= abs(vect[,i]))
	write.table(t(pval),file="ppc_SC_Goodness.txt", append=T,quote=F,row.names=mod_names[i], col.names=F)
	}
dev.off()
