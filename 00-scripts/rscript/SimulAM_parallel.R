##############################################################################
# full script to perform simulations based on ANCIENT MIGRATION MODEL using ms#
##############################################################################
...
if (!add_simulations){
  write.table( cbind( #### PARAMETERS
    ...
    "tau_ema",
    "ema12",	  # ancient migration (lp (2) -> lf(1))
    "ema21",      # ancient migration (lf ->lp)
    "tau",        # split time
    #### SUMMARY STATISTICS                      
    "mean_H_pop1",
    "var_H_pop1",
...
for(i in 1:nsim){
  ...
  thetaRef=1.2 #hypothèse: Ne=2000 mu=1.5*10^-4
  
  theta1_min <- 0 
  #theta2_min=theta1_min
  theta1_max <- 3
  thetaA_max <- 5
  model <- 2 
  #0: I 
  ...
  tau_ema = runif(1, tmin, tau)
  
 ema12 <- runif (1, min =0, max=15)
 ema21 <- runif (1, min=0, max=15)

  #Probabilité Single Insertion déletion 
  #Psni  <- runif( 1, min=0, max=0.1 )
  #probab of mutation changing the number of repeats
 ...
 deltamu2 <- array(NA,number_of_loci)
  ...
  for (locus in 1:number_of_loci){
    ...
    ms_run <- paste(sample_size_total[locus], "1",       # total sample size
                     ...
                     "-ema", tau_ema, "2 0", ema12, ema21, "0",	# ancient migration AM)
                     "-n 1", theta1,                             # size pop 2
                     "-n 2", theta2,                       # size ancestral pop(pop 1)
    ...
    if(.Platform$OS.type == "unix") {
      ...
      #ms_run <- paste( "/usr/local/bioinfo/src/ms/msdir/./ms", ms_run, sep=" ") #rennes  
      system( ms_run )
    }else{
      shell( ms_run )
    }
 ...
 # add the mutationns to ms output (microsatellites transformations)   
    ...
    if  (length(motif_size[[locus]])==1){
      mutation_size <- (rgeom(mutations,alpha)+1) * motif_size[[locus]] #follow a strict SMM and multiply by loci length(i.e 1)
    }else{
    ...
    genotypes <- 500+colSums(t(msout$gametes[[1]])*mutation_size) #On transpose pour pouvoir mutliplier les 179 mutations * le nombre de segregation sites pour chaque individus
  ...
  write.table( cbind( ## PARAMETERS
    ...
    #mig21,          # migration (lf -> lp)
    tau_ema,        # end of ancien migration
    ema12,	    # ancient migration rate from 2 (lp) to 1 (lf)
    ema21,          # ancient migration rate from 1 (lf) to 2 (lp)
    tau,            # split time
    ## SUMMARY STATISTICS
    mean_H_pop1,
    var_H_pop1,
