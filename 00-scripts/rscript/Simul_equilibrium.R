##################################################################################
# full script to perform simulations based on ISOLATION W MIGRATION MODEL using ms#
##################################################################################
...
for(i in 1:nsim){
  ...
  thetaRef=2.5 #hypothèse: Ne=50 000 mu=1.25*10^-5
  theta1_min <- 0 
  theta1_max <- 10
  #thetaA_max <- 25
 ...
 deltamu2 <- array(NA,number_of_loci)
  ...
  for (locus in 1:number_of_loci){
    ...
    if(.Platform$OS.type == "unix") {
      #ms_run <- paste( "/home/qrougemo/bin/ms_test/msdir/./", ms_run, sep=" ") #unil
