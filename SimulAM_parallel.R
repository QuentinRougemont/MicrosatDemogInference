...
if (!add_simulations){
  write.table( cbind( #### PARAMETERS
    ...
    "tau_ema",    # time of ancient migration
    "ema12",      # ancient migration (l. planeri(2) -> L. fluviatilis (1))
...
for(i in 1:nsim){
  ...
  tau_ema = runif(1, tmin, tau) #time of ancient migration stop
  
  # ancient migration :
 ema12 <- runif (1, min =0, max=20)
 ema21 <- runif (1, min=0, max=20)
 ...
 deltamu2 <- array(NA,number_of_loci)
  ...
  for (locus in 1:number_of_loci){
    ...
    ms_run <- paste(sample_size_total[locus], "1",                # total sample size
                     "-t", thetaRef,                              # theta Ref (arbitrarrily fixed)
                     ...
                     "-n 1", theta1,                              # size pop 1
                     "-n 2", theta2,                              # size ancestral pop 2
