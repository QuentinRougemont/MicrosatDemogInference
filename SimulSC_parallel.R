...
for(i in 1:nsim){
  ...
  theta2=runif(1,theta1_min, theta1) #constrained based on the lower diversity of Lp as compared to lf (cf Rougemont et al. JEB, 2015).
 ...
 #Time of begining of secondary contact (constrained on tau)
 ...
 deltamu2 <- array(NA,number_of_loci)
  ...
  for (locus in 1:number_of_loci){
    ...
    ms_run <- paste(sample_size_total[locus], "1",                # total sample size
                     ...
                     "-eM" , tau_eM, "0",                         # Time of secondary contact
                     "-ej", tau, "2 1",                           #migration sets to Zero at the split time tau
                     "-eN" , tau, thetaA,                         #Size of the ancestral pop at split time tau
