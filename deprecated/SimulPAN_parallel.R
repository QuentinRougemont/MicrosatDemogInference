##############################################################################
# full script to perform simulations based on PANMIXIA MODEL using ms#
##############################################################################
...
# SIMULATION
#########################################
#IM: ms [total sample size] 1 -t [theta ref] -I 2 [sample size pop 1] [sample size pop 2] 0 -m 1 2 [mig12] -m 2 1 [mig21] -n 1 [theta 1] -n 2 [theta 2] -ej [tau] 2 1 -eN [tau] [theta A]
...
for(i in 1:nsim){
  ...
  model <- 4 #index of the model
  #0: I 1: IM 2: AM 3: SC 4: PAN
  
  theta1=NULL #theta single panmictic population
  
  tmp = runif(1, theta1_min, theta1_max)
 ...
 deltamu2 <- array(NA,number_of_loci)
  ...
  for (locus in 1:number_of_loci){
    ...
    ms_run <- paste(sample_size_total[locus], "1",                           # total sample size
                     "-t", theta1,                                           # size population1+2 (panmictic pop)
                     "-seeds","${JOB_ID}","${SGE_TASK_ID}","${SGE_TASK_ID}", #random seed 
